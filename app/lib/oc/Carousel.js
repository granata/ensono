...
function customcarousel() {
    ...
    function template() {
        return [
            ...
            '@media (min-width:768px){.carousel-control.left {background-image: none;margin-left:-2%;}.carousel-control.right {background-image: none;margin-right:-4%;}}',
            '</style>',
...
function customCarouselCtrl($scope, $animate, $filter) {
    ...
    $scope.$watch('user.CustomFields', function(newVal){
        ...
        $scope.slides = $scope.slides.concat($filter('carouselFilter')($scope.user.CustomFields, $scope.user.Company.Name + '-slide'));
